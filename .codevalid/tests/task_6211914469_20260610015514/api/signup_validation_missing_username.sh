...
CASE_SUFFIX="$(date +%s)-$$"
EMAIL="validuser_${CASE_SUFFIX}@example.com"
PASSWORD="ValidPass789!"
FULL_NAME="Valid User Name ${CASE_SUFFIX}"
RESPONSE_FILE="/tmp/signup_validation_missing_username_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/signup_validation_missing_username_${CASE_SUFFIX}.status"
cleanup_files() {
  rm -f "$RESPONSE_FILE" "$STATUS_FILE"
}
trap cleanup_files EXIT

# Given — prepare signup payload with missing username
:
...
echo "CODEVALID_TEST_ASSERTION_OK:signup_validation_missing_username"
