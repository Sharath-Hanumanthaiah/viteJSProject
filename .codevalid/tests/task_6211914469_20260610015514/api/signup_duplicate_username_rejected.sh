...
NEW_EMAIL="newemail_${CASE_SUFFIX}@test.com"
PASSWORD="NewPass456!"
...
# When — submit a second signup with a duplicate username and unique email
...
# Then — duplicate username is rejected
STATUS="$(cat "$STATUS_FILE")"
[ "$STATUS" = "400" ]
grep -F '"message":"Username or Email already registered."' "$RESPONSE_FILE" >/dev/null

echo "CODEVALID_TEST_ASSERTION_OK:signup_duplicate_username_rejected"
