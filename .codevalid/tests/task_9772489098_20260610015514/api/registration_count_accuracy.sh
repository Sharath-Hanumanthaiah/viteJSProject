...
RESPONSE_FILE="/tmp/registration_count_accuracy_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/registration_count_accuracy_${CASE_SUFFIX}.status"
cleanup_files() { rm -f "$RESPONSE_FILE" "$STATUS_FILE"; }
trap cleanup_files EXIT

# Given — use seeded registrations where event_1 has 2 registrations and event_2/event_3 have none

# When — request the events list
curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' \
  "$BASE_URL/api/events" > "$STATUS_FILE"

# Then — verify registrationCount values reflect the seeded registrations data
