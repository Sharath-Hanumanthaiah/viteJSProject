...
test("Unauthenticated Access to Protected Route Redirects to Sign In", async ({ page }, testInfo) => {
  ...
  await recorder.step("Register safe default mocks for protected home page APIs", async () => {
    await mockProtectedHomePageApis(page, { events: [] });
  });

  await recorder.step("Navigate directly to the protected root route", async () => {
    await page.goto("/");
  });

  await recorder.step("Verify the user is redirected to the sign-in page", async () => {
    await expect(page).toHaveURL(/\/signin$/);
    await expect(page.getByRole("heading", { name: "Welcome Back" })).toBeVisible();
    await expect(page.getByPlaceholder("john@example.com")).toBeVisible();
    await expect(page.getByRole("button", { name: /sign in/i })).toBeVisible();
    await expect(page.getByText("Registration Desk")).toHaveCount(0);
  });
